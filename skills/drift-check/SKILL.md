...
5. **Cross-check the lock files.** `npx -p regen-engineering-schema regen-debt .` reports declared drift and, in a git repository, compares each lock's `knowledge_version` against the last commit touching that module's knowledge.
